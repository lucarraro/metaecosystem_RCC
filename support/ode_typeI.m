...
function dy=eqs(t,y)
    ...
    dy(ind_P) = epsilonP*alphaP*light.*y(ind_P).*y(ind_N) - alphaG*y(ind_P) .*y(ind_G) - muP*y(ind_P) - betaP*y(ind_P).*y(ind_P); 
    ...
    dy(ind_N) = 86400*deltaN*(Wt*(y(ind_N).*Q) - y(ind_N).*Q)./V +...
        kN*aN.*AS./V - alphaP*light.*y(ind_N).*y(ind_P) - lambdaN*y(ind_N);

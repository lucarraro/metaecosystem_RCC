...
Bbar=5;
% patch for aF varying with elevation
%aFpixel=0.05+0.9*Z/max(Z);
aFpixel=double(Z>=1000 & Z<2000); aFpixel(aFpixel==0)=0.01;
aNpixel=double(Z<1000); aNpixel(aNpixel==0)=0.01;
aFscenario=zeros(N_reach,1); aNscenario=zeros(N_reach,1);
Z_subcat=zeros(N_reach,1);
for i=1:N_reach
    tmp=find(subcatchMat==i);
    aFscenario(i)=mean(aFpixel(tmp));
    aNscenario(i)=mean(aNpixel(tmp));
    Z_subcat(i)=mean(Z(tmp));
end
if strcmp(scenario,'flat')
    aF=0.5*ones(N_reach,1); aN=1-aF;
else
    aF=aFscenario; aN=aNscenario;
end
Beff = min(B,Bbar);
...
% disp(sprintf('noUpstream simulation'))
% parameters_noUpstream=parameters;
% parameters_noUpstream.zeta=0;
% [X_noUpstream,flag_converge,maxreltol,Niter_noUpstream] = linearized_typeI(parameters_noUpstream,OCNparam);
...
binplot(A,X_noFlow(ind_R),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_R),[0.25 0.5 0],type,3);
...
%text(2e6,5e-7,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_R).*V),100*sum(X_noUpstream(ind_R).*V)/sum(X(ind_R).*V)),'Color',[0.25 0.5 0]);
...
binplot(A,X_noFlow(ind_S),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_S),[0.25 0.5 0],type,3);
...
%text(2e6,4e-5,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_S).*V),100*sum(X_noUpstream(ind_S).*V)/sum(X(ind_S).*V)),'Color',[0.25 0.5 0]);
subplot(2,5,3); binplot(A,X(ind_C),'k',type,3); title('Collectors'); ylabel('Density [m^{-3}]')
set(gca,'xlim',[1e6 1e10],'ylim',[1e-8 1e-4],'yscale','log'); hold on;
binplot(A,X_noFlow(ind_C),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_C),[0.25 0.5 0],type,3);
...
%text(2e6,4e-5,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_C).*V),100*sum(X_noUpstream(ind_C).*V)/sum(X(ind_C).*V)),'Color',[0.25 0.5 0]);
subplot(2,5,4); binplot(A,X(ind_F),'k',type,3); title('Filter feeders'); ylabel('Density [m^{-3}]')
set(gca,'xlim',[1e6 1e10],'ylim',[1e-8 1e-4],'yscale','log'); hold on;
binplot(A,X_noFlow(ind_F),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_F),[0.25 0.5 0],type,3);
...
%text(2e6,4e-5,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_F).*V),100*sum(X_noUpstream(ind_F).*V)/sum(X(ind_F).*V)),'Color',[0.25 0.5 0]);
subplot(2,5,5); l1=binplot(A,X(ind_G),'k',type,3); title('Grazers'); ylabel('Density [m^{-3}]')
set(gca,'xlim',[1e6 1e10],'ylim',[1e-8 1e-4],'yscale','log'); hold on;
l2=binplot(A,X_noFlow(ind_G),[0.5 0 0.5],type,3); %l3=binplot(A,X_noUpstream(ind_G),[0.25 0.5 0],type,3);
...
%text(2e6,4e-5,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_G).*V),100*sum(X_noUpstream(ind_G).*V)/sum(X(ind_G).*V)),'Color',[0.25 0.5 0]);
...
binplot(A,X_noFlow(ind_MC),[0.5 0 0.5],type,3);  %binplot(A,X_noUpstream(ind_MC),[0.25 0.5 0],type,3); xlabel('Drainage Area [m^2]')
text(2e6,8e-3,sprintf('%.3e',sum(X(ind_MC).*V)));
text(2e6,6.5e-3,sprintf('%.2e (%.1f%%)',sum(X_noFlow(ind_MC).*V),100*sum(X_noFlow(ind_MC).*V)/sum(X(ind_MC).*V)),'Color',[0.5 0 0.5]);
%text(2e6,5.5e-3,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_MC).*V),100*sum(X_noUpstream(ind_MC).*V)/sum(X(ind_MC).*V)),'Color',[0.25 0.5 0]);
...
binplot(A,X_noFlow(ind_MF),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_MF),[0.25 0.5 0],type,3); xlabel('Drainage Area [m^2]')
...
%text(2e6,4e-4,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_MF).*V),100*sum(X_noUpstream(ind_MF).*V)/sum(X(ind_MF).*V)),'Color',[0.25 0.5 0]);
...
binplot(A,X_noFlow(ind_MD),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_MD),[0.25 0.5 0],type,3); xlabel('Drainage Area [m^2]')
...
%text(1e8,2.25e-1,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_MD).*V),100*sum(X_noUpstream(ind_MD).*V)/sum(X(ind_MD).*V)),'Color',[0.25 0.5 0]);
subplot(2,5,9); binplot(A,X(ind_N),'k',type,3); title('Nutrients'); ylabel('Density [m^{-3}]')
set(gca,'xlim',[1e6 1e10],'ylim',[9.999e-6 1e-2],'yscale','log'); hold on;
binplot(A,X_noFlow(ind_N),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_N),[0.25 0.5 0],type,3); xlabel('Drainage Area [m^2]')
...
%text(1e8,2.25e-1,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_N).*V),100*sum(X_noUpstream(ind_N).*V)/sum(X(ind_N).*V)),'Color',[0.25 0.5 0]);
...
binplot(A,X_noFlow(ind_P),[0.5 0 0.5],type,3); %binplot(A,X_noUpstream(ind_P),[0.25 0.5 0],type,3); xlabel('Drainage Area [m^2]')
...
%text(1e8,5e-5,sprintf('%.2e (%.1f%%)',sum(X_noUpstream(ind_P).*V),100*sum(X_noUpstream(ind_P).*V)/sum(X(ind_P).*V)),'Color',[0.25 0.5 0]);
